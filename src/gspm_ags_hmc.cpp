...
List gspm_ags_hmc(
    ...
    double lambda,
    double sigma,
    unsigned int sample_sigma,
    unsigned int sigma_update_iter,
    double gamma_shape,
    double gamma_rate,
  ...
  arma::mat Sigma_samples = zeros<mat>(M, max_iter);
  // vec diag_sigma = Rcpp::as<arma::vec>(Rcpp::rgamma(M, gamma_shape, 1. / gamma_rate));
  vec diag_sigma = ones<vec>(M) * sigma; // initialize the symmetric diagonal variance
  vec diag_precision = pow(diag_sigma, -2.);
  ...
  double gamma_post_shape = (n_samples / 2.) + gamma_shape;
  vec gamma_post_rate = zeros<vec>(M);
  ...
  for (iter = 0; iter < max_iter; iter++) { // for each HMC within AGS iteration
    ...
    Sigma_samples.col(iter) = diag_sigma;
    ...
    for (j = 0; j < M; j++) { // for each gene or feature
      ...
      for (k = 0; k < K; k++) { // for each subtype
        ...
        mu_jk = B_jk / (((diag_sigma(j) * diag_sigma(j)) / C_jk) + A(k)); // mean
        sigma_jk = 1. / sqrt((1. / C_jk) + (A(k) / (diag_sigma(j) * diag_sigma(j)))); // s.d.
    ...
    gamma_post_rate.fill(gamma_rate); // prior hyperparameter
    yut = 0.;
    for (i = 0; i < n_samples; i++) { // for each observation
      yut += arma::accu(
        arma::pow((trans(Y.row(i)) - U_Theta_sum.col(i)) / diag_sigma, 2.)
      );
      gamma_post_rate += .5 * arma::pow(trans(Y.row(i)) - U_Theta_sum.col(i), 2.);
    ...
    // Posterior sampling of \rho_j = 1 / \sigma_j given U, Theta, and Y, for
    // each gene or feature j
    //
    // Note: We sample precision only after certain number of iterations
    if (sample_sigma == 1 && iter >= sigma_update_iter) {
      for (j = 0; j < M; j++) {
        diag_precision(j) = Rcpp::rgamma(
          1, // n
          gamma_post_shape, // shape
          1. / gamma_post_rate(j) // scale
        )(0);
        diag_sigma(j) = 1. / sqrt(diag_precision(j));
      }
    }
  ...
  return List::create(
    ...
    Named("Sigma_samples") = wrap(Sigma_samples)
  );
